...
class Fourier{
    ...
    public:
        Fourier(size_t PS){
            ...
            Nx=PS;
            size_t totalValues=Nx*Nx*Nx;
            // The one-dimensional array which goes on fftw functions
                    ...
                    for(size_t i=0;i<Nx;i++){
                        for(size_t j=0; j<Nx;j++){
                            for(size_t k=0; k<Nx;k++){
                                ...
                                double momentum2=
                                    pow(2*M_PI/Length,2)*(pow(shift(i,Nx),2) + pow(shift(j,Nx),2) + pow(shift(k,Nx),2));
...
//input psi on the array for fftw
        void inputpsi(multi_array<double,4> &psi){
            size_t i,j,k;
            #pragma omp parallel for collapse(3)
            for(i=0;i<Nx;i++){
                for(j=0; j<Nx;j++){
                    for(k=0; k<Nx;k++){
                        rin[i+Nx*j+Nx*Nx*k][0]= psi[0][i][j][k];
                        rin[i+Nx*j+Nx*Nx*k][1]= psi[1][i][j][k];
                    }
                }
            }
            #pragma omp barrier
        ...
        void kfactorpsi(double tstep, double Length, double calpha){
            ...
            for(i=0;i<Nx;i++)
              for(j=0; j<Nx;j++)
                 for(k=0; k<Nx;k++){
                    double ksq= -0.5*tstep*calpha*(pow(shift(i,Nx),2)+pow(shift(j,Nx),2)+pow(shift(k,Nx),2))*4*M_PI*M_PI/(Length*Length);
        ...
        void transferpsi(multi_array<double,4> &psi, double factor){
            size_t i,j,k;
            #pragma omp parallel for collapse(3)
            for(i=0;i<Nx;i++)
              for(j=0; j<Nx;j++)
                 for(k=0; k<Nx;k++){
                    psi[0][i][j][k]=rin[i+Nx*j+Nx*Nx*k][0]*factor;
                    psi[1][i][j][k]=rin[i+Nx*j+Nx*Nx*k][1]*factor;
                    }
            #pragma omp barrier
        }
       ...
       virtual void inputPhi(multi_array<double,4> &psi_in){ // Luca: I removed the psisqmean, since it is useless
            #pragma omp parallel for collapse(3)
               for(size_t i=0;i<Nx;i++)
                  for(size_t j=0; j<Nx;j++)
                     for(size_t k=0; k<Nx;k++){
                        rin[i+Nx*j+Nx*Nx*k][0]=pow(psi_in[0][i][j][k],2)+pow(psi_in[1][i][j][k],2);
       ...
       void kfactorPhi(double Length){
            ...
            for(size_t i=0;i<Nx;i++)
              for(size_t j=0; j<Nx;j++)
                 for(size_t k=0; k<Nx;k++){
                    double ksq=(pow(shift(i,Nx),2)+pow(shift(j,Nx),2)+pow(shift(k,Nx),2))*4*M_PI*M_PI/(Length*Length);
                    if(ksq==0){
                        rin[i+Nx*j+Nx*Nx*k][1]= 0;
       ...
       double e_kin_FT(multi_array<double,4> &psi, double Length){
            inputpsi(psi);
            calculateFT();
...
class domain3{
  protected:
    ...
    bool Grid3D = true; // If true, it outputs the full density on the 3D grid; if false (recommended), it outputs the 2D projection of the density profile
    ...
    public:
        domain3(size_t PS, double L, int Numsteps, double DT, int Nout, int Nout_profile, string Outputname, int pointsm):
            psi(extents[2][PS][PS][PS]),     //real and imaginary parts are stored consequently, i.e. psi[0]=real part and psi[1]=imaginary part
            Phi(extents[PS][PS][PS]),
            fgrid(PS), // class for Fourier trasform, defined above
            ca(extents[8]),
            da(extents[8]),
            PointsS(PS),
            Length(L),
...
//         void closefiles(){
        ...
        void outputfulldensity(ofstream& fileout){// Outputs the full 3D density profile
                multi_array<double,3> density(extents[PointsS][PointsS][PointsS]);
                #pragma omp parallel for collapse(3)
                    for(size_t i=0;i<PointsS;i++)
                        for(size_t j=0; j<PointsS;j++)
                            for(size_t k=0; k<PointsS;k++){
                                density[i][j][k]= pow(psi[0][i][j][k],2)+pow(psi[1][i][j][k],2);
                            }
                #pragma omp barrier
        ...
        void outputSlicedDensity(ofstream& fileout){ // Outputs the projected 2D density profile
                multi_array<double,2> density_sliced(extents[PointsS][PointsS]);
                #pragma omp parallel for collapse(3)
                    for(size_t i=0;i<PointsS;i++)
                        for(size_t j=0; j<PointsS;j++)
                            for(size_t k=0; k<PointsS;k++){
                                density_sliced[i][j]= density_sliced[i][j] + pow(psi[0][i][j][k],2)+pow(psi[1][i][j][k],2);
                            }
                #pragma omp barrier
                print2(density_sliced,fileout);
    }

        void outputPhaseSlice(ofstream& fileout){ // Outputs a 2D slice of the phase
                multi_array<double,2> phase_sliced(extents[PointsS][PointsS]);
                #pragma omp parallel for collapse(2)
                    for(size_t i=0;i<PointsS;i++)
                        for(size_t j=0; j<PointsS;j++){
                                phase_sliced[i][j]= atan2(psi[1][i][j][int(PointsS/2)], psi[0][i][j][int(PointsS/2)]);
                            }
                #pragma omp barrier
                print2(phase_sliced,fileout);
    }

        long double psisqmean(){// Computes the mean |psi|^2
            long double totV=0;
            #pragma omp parallel for collapse(3) reduction(+:totV)
            for(size_t i=0;i<PointsS;i++)
                for(size_t j=0;j<PointsS;j++)
                    for(size_t k=0;k<PointsS;k++)
                        totV=totV+pow(psi[0][i][j][k],2)+pow(psi[1][i][j][k],2);
            #pragma omp barrier
            totV=totV/(PointsS*PointsS*PointsS);
            return totV;
        }

        double total_mass(){ // Computes the total mass
          return psisqmean() * pow(Length, 3);
        }

        inline double energy_kin(const int & i, const int & j, const int & k){// it computes the kinetic energy density at grid point (i,j,k)
          double der_psi1re = derivative_3point(psi[0][cyc(i+1, PointsS)][j][k], psi[0][cyc(i-1, PointsS)][j][k], psi[0][cyc(i+2, PointsS)][j][k],
              psi[0][cyc(i-2, PointsS)][j][k], psi[0][cyc(i+3, PointsS)][j][k], psi[0][cyc(i-3, PointsS)][j][k])/deltaX;
          double der_psi1im = derivative_3point(psi[1][cyc(i+1, PointsS)][j][k], psi[1][cyc(i-1, PointsS)][j][k], psi[1][cyc(i+2, PointsS)][j][k],
              psi[1][cyc(i-2, PointsS)][j][k],psi[1][cyc(i+3, PointsS)][j][k], psi[1][cyc(i-3, PointsS)][j][k])/deltaX;
          double der_psi2re = derivative_3point(psi[0][i][cyc(j+1, PointsS)][k], psi[0][i][cyc(j-1, PointsS)][k], psi[0][i][cyc(j+2, PointsS)][k],
              psi[0][i][cyc(j-2, PointsS)][k], psi[0][i][cyc(j+3, PointsS)][k], psi[0][i][cyc(j-3, PointsS)][k])/deltaX;
          double der_psi2im = derivative_3point(psi[1][i][cyc(j+1, PointsS)][k], psi[1][i][cyc(j-1, PointsS)][k], psi[1][i][cyc(j+2, PointsS)][k],
              psi[1][i][cyc(j-2, PointsS)][k], psi[1][i][cyc(j+3, PointsS)][k], psi[1][i][cyc(j-3, PointsS)][k])/deltaX;
          double der_psi3re = derivative_3point(psi[0][i][j][cyc(k+1, PointsS)], psi[0][i][j][cyc(k-1, PointsS)], psi[0][i][j][cyc(k+2, PointsS)],
              psi[0][i][j][cyc(k-2, PointsS)], psi[0][i][j][cyc(k+3, PointsS)], psi[0][i][j][cyc(k-3, PointsS)])/deltaX;
          double der_psi3im = derivative_3point(psi[1][i][j][cyc(k+1, PointsS)], psi[1][i][j][cyc(k-1, PointsS)], psi[1][i][j][cyc(k+2, PointsS)],
              psi[1][i][j][cyc(k-2, PointsS)], psi[1][i][j][cyc(k+3, PointsS)], psi[1][i][j][cyc(k-3, PointsS)])/deltaX;
        ...
        virtual inline double energy_pot(const int & i, const int & j, const int & k){
            return 0.5*(pow(psi[0][i][j][k],2) + pow(psi[1][i][j][k],2))*Phi[i][j][k];
        }

        double e_kin_full1(){//Full kinetic energy with Fourier
            return pow(Length,3)*fgrid.e_kin_FT(psi,Length);
        }

        long double full_energy_kin(){// it computes the kinetic energy of the whole box with derivatives
          ...
          for(int i=0;i<PointsS;i++)
                for(int j=0; j<PointsS;j++)
                    for(int k=0; k<PointsS;k++){
                      total_energy = total_energy + energy_kin(i,j,k);
                    }
          return total_energy*pow(Length/PointsS,3);
        }

        long double full_energy_pot(){// it computes the potential energy of the whole box
          long double total_energy = 0;
          #pragma omp parallel for collapse (3) reduction(+:total_energy)
          for(int i=0;i<PointsS;i++)
                for(int j=0; j<PointsS;j++)
                    for(int k=0; k<PointsS;k++){
                      total_energy = total_energy + energy_pot(i,j,k);
                    }
          return total_energy*pow(Length/PointsS,3);
        }

        double find_maximum(){ // Sets maxx, maxy, maxz equal to the maximum, it just checks for one global maximum
          double density_max = 0;
          for(int i=0;i<PointsS;i++)
                for(int j=0; j<PointsS;j++)
                    for(int k=0; k<PointsS;k++){
                      double density_current = pow(psi[0][i][j][k],2) + pow(psi[1][i][j][k],2);
                      if (density_current > density_max)
                      {maxx=i; maxy=j; maxz=k; density_max =density_current; }
                    }
          return density_max;
        }
            ...
            for(int i=0;i<PointsS;i++)
              for(int j=0; j<PointsS;j++)
                for(int k=0; k<PointsS;k++){
                    ...
                    int Dz=maxz-(int)k; if(abs(Dz)>PointsS/2){Dz=abs(Dz)-(int)PointsS;} // periodic boundary conditions!
                    ...
                    for(size_t i=0;i<PointsS;i++)
                        for(size_t j=0;j<PointsS;j++)
                            for(size_t k=0;k<PointsS;k++){
                                ...
                                psi[0][i][j][k]=cos(-tstep*da*Phi[i][j][k])*Repsi - sin(-tstep*da*Phi[i][j][k])*Impsi;   //real part
                                psi[1][i][j][k]=sin(-tstep*da*Phi[i][j][k])*Repsi + cos(-tstep*da*Phi[i][j][k])*Impsi;   //im part
                        }
                    #pragma omp barrier
        }

        void snapshot(double stepCurrent){//Outputs the full density profile; if 3dgrid is false, it outputs just sliced density
            cout.setf(ios_base::fixed);
            timesfile_grid<<"{"<<tcurrent<<","<<maxdensity<<","<<maxx<<"," <<maxy<<","<<maxz<<","
              <<e_kin_full1()<<","<<full_energy_pot() << ","<<total_mass()<<"}"<<endl;
            ...
            if(Grid3D == true){
              ...
              cout.setf(ios_base::fixed);
              outputSlicedDensity(profile_sliced);
            if(phaseGrid == true){
              cout.setf(ios_base::fixed);
        ...
        void snapshot_profile(double stepCurrent){// Outputs only few relevant information like mass, energy, radial profiles
            cout.setf(ios_base::fixed);
            maxdensity = find_maximum();
            // cout<<"before profile = "<< time(NULL)- beginning <<endl;
            multi_array<double,2> profile = profile_density(maxdensity);
            // cout<<"after profile = "<< time(NULL)- beginning <<endl;
            print2(profile,profilefile);
            long double e_pot_full = full_energy_pot();
            long double e_kin_full_1 = full_energy_kin();
            long double e_kin_full_FT = e_kin_full1();
            timesfile_profile<<"{"<<tcurrent<<","<<maxdensity<<","<<maxx<<"," <<maxy<<","<<maxz<<","
              <<e_kin_full_FT<< ","<< e_kin_full_1 << "," << e_pot_full <<","<<e_kin_full_FT + e_pot_full
              << ","<< e_kin_full_1 + e_pot_full <<","<<total_mass() <<"}"<<endl;
            // if(stepCurrent<numsteps){
        ...
        virtual void makestep(double stepCurrent, double tstep){ // makes a step in a dt
            ...
            for(int alpha=0;alpha<8;alpha++){
                ...
                fgrid.transferpsi(psi,1./pow(PointsS,3));                             //divides it by 1/PS^3 (to get the correct normalizaiton of the FT)
                ...
                if(alpha!=7){  //da[7]=0 so no evolution here anyway
                    ...
                    fgrid.inputPhi(psi);                                     //inputs |psi^2|
            ...
            cout<<"mean value "<<psisqmean() <<endl;
        }
        ...
        void solveConvDif(){
            ...
            if (start_from_backup == false){
              ...
              snapshot_profile(stepCurrent); // I want the snapshot of the initial conditions
            ...
            while(stepCurrent<=numsteps){
                ...
                for(int index=0;index<=numoutputs_profile;index++)
                    if(stepCurrent==jumps_profile[index]) {
                      ...
                      double etot_current = e_kin_full1() + full_energy_pot();
...
// Initial conditions functions
        ...
        void initial_waves(){
            ...
            for(size_t i=0;i<PointsS;i++)
                for(size_t j=0;j<PointsS;j++)
                    for(size_t k=0;k<PointsS;k++){
                        psi[0][i][j][k]=sin(i/20.)*cos(k/20.);
                        psi[1][i][j][k]=sin(k/20.)*cos((k+i)/20.);
                    }
        }
        ...
        void set_waves_Levkov(double Npart, double vw){
          // Sets waves initial conditions a la Levkov, Npart is number of particles, vw is velocity (set it to zero)
            ...
            fgrid.calculateFT();
            fgrid.transferpsi(psi, 1./pow(Length,3));
