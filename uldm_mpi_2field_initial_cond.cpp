...
void domain3::setInitialSoliton_1(double r_c, int whichpsi){ // sets 1 soliton in the center of the grid as initial condition, for field 0
  ...
  int extrak;
  if (mpi_bool==true){
    extrak= PointsSS*(world_rank-maxNode); // Take into account the node where you are
  }
  else {
    extrak = 0;
  }
...
void domain3::setManySolitons_random_radius(int num_Sol, double min_radius, double max_radius, double length_lim){
  int extrak; // Take into account the node where you are
  if (mpi_bool==true){
    extrak= PointsSS*(world_rank-maxNode); // Take into account the node where you are
  }
  else {
...
void domain3::set_waves_Levkov(double Npart, double Npart1, double vw){
  // Sets waves initial conditions a la Levkov, Npart is number of particles, Npart1 number of field 1 particles 
  // vw is velocity (set it to zero)
  // Ratio is the mass ratio between the two masses
  fgrid.inputSpectrum(Length, Npart,vw);
  fgrid.calculateFT();
  fgrid.transferpsi(psi, 1./pow(Length,3),nghost,0);

  fgrid.inputSpectrum(Length, Npart1,vw);
  fgrid.calculateFT();
  fgrid.transferpsi(psi, 1./pow(Length,3),nghost,1);

  if(world_rank==0){
    info_initial_cond.open(outputname+"initial_cond_info.txt");
    info_initial_cond.setf(ios_base::fixed); // First row of the file is num_sol, r_c and length_lim, the other rows are centers of solitons
    info_initial_cond<<"{"<<Npart<<"," <<Npart1<<"," <<vw << "}" <<endl;
    info_initial_cond.close();
  }
}