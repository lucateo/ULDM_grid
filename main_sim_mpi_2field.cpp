...
int main(int argc, char** argv){
  ...
  if (initial_cond == "levkov" ) {// Levkov initial conditions
    if (params_initial_cond.size() > 2*num_fields-2){
      ...
      run_ok = false; 
      if (world_rank==0)
      cout<<"You need 2*num_fields-1 arguments to pass to the code: {Npart}, {ratio_masses without for field 0}" << endl;
    }
  }
    ...
    if (params_initial_cond.size() > 2){
      ...
      outputname = "out_mpi/out_test/out_1Sol" + outputname + to_string(ratio_mass[whichpsi])+ "_rc_" 
    ...
    if (params_initial_cond.size() > 3*(num_fields-1)){
      outputname ="out_Eddington/Eddington_NFW" + outputname ;
      double rs [num_fields];
      double rhos [num_fields];
      for(int i = 0; i < num_fields; i++){
        ratio_mass[i] = stod(params_initial_cond[3*i]); // If nfields=1, this should be set to one
        rs[i] = stod(params_initial_cond[1 + 3*i]); // NFW scale radius
        rhos[i] = stod(params_initial_cond[2 + 3*i]); // NFW normalization
        outputname = outputname + "rmass_"+to_string(ratio_mass[i]) + "_rs_" + to_string(rs[i]) + "_rhos_" + to_string(rhos[i])+ "_";
      }
      int num_k = stoi(params_initial_cond[3 + 3*(num_fields-1)]); // Number of maximum k points in nested loop
      bool boolk = false; // simplify k sum always false
      ...
      run_ok=false; 
      if (world_rank==0)
        cout<<"You need (3*nfields+1) arguments to pass to the code: [ratio_mass[i], rs[i], rhos[i]], num_k" << endl;
    }
  }
    ...
    if (params_initial_cond.size() > 5){
      ...
      ratio_mass[1] = stod(params_initial_cond[1]); // If nfields=1, this should be set to one
    ...
    if (params_initial_cond.size() > 3){
      ...
      run_ok=false; 
      if (world_rank==0)
        cout<<"You need 3 arguments to pass to the code: rc, Num_sol, length_lim" << endl;
    }
  }
    ...
    if (params_initial_cond.size() > 2){
      double rc = stod(params_initial_cond[0]); //radius of soliton
      int Nsol = stod(params_initial_cond[1]); // Number of solitons, should not surpass 30
      outputname = "out_test/out_deterministic"+ outputname + "_rc_" + to_string(rc)+ "_Nsol_" + to_string(Nsol)+"_";
      domain3 D3(Nx,Nz,Length,num_fields,numsteps,dt,outputnumb, outputnumb_profile, outputname, Pointsmax, world_rank,world_size,nghost,mpirun_flag);
      if(start_from_backup=="true")
        D3.initial_cond_from_backup();
      else
      ...
      run_ok=false; 
      if (world_rank==0)
        cout<<"You need 2 arguments to pass to the code:rc, Nsol " << endl;
    }
  }
    ...
    if (world_rank==0){
      ...
      cout<< "Schive , Mocz , deterministic , levkov, delta, theta, 1Sol, NFW, NFW_solitons, NFW_ext_Eddington, eddington_nfw, eddington_nfw_levkov, eddington_nfw_soliton, stars, stars_soliton" <<endl;
    }
  }
